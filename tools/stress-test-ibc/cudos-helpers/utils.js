...
export async function getCudosAddressBalance(address, url) {
    const res = await fetch(url + "/cosmos/bank/v1beta1/balances/" + address + '/acudos');
...
export async function getIbcDenom(address, url) {
    const res = await fetch(url + "/cosmos/bank/v1beta1/balances/" + address);

    const data = await res.json();
    const denom = data.balances.find(b => b.denom !== 'acudos').denom;
    return  denom;
}
}

export async function getTotalDenomBalance(denom, url) {
    const res = await fetch(url + "/cosmos/bank/v1beta1/supply/" + denom);
