...
dockerResult=$(docker-compose --env-file ./$NODE_NAME.client.mainnet.arg -f ./init-$NODE_NAME.yml -p cudos-init-$NODE_NAME-client-mainnet up --build 2>&1)
if [ "$?" != 0 ]; then
    echo -e "${STYLE_RED}Error:${STYLE_DEFAULT} There was an error building the container $?: ${dockerResult}";
    exit 1;
fi
...
if [ $IS_VALIDATOR = "true" ]; then
    ...
    dockerResult=$(docker-compose --env-file ./full-node.client.mainnet.arg -f ./start-full-node.yml -p cudos-start-full-node-client-mainnet-01 down 2>&1);

    sed -i "s/cudos-noded start/sleep infinity/g" "./start-full-node.dockerfile";
    dockerResult=$(docker-compose --env-file ./full-node.client.mainnet.arg -f ./start-full-node.yml -p cudos-start-full-node-client-mainnet-01 up --build -d 2>&1);
    if [ "$?" != 0 ]; then
        echo -e "${STYLE_RED}Error:${STYLE_DEFAULT} There was an error building the container $?: ${dockerResult}";
        exit 1;
    fi;
    ...
    dockerResult=$(docker container exec "$startContainerName" /bin/bash -c "cudos-noded init \$MONIKER --chain-id $chainId" 2>&1);
    ...
    dockerResult=$(docker container exec "$startContainerName" /bin/bash -c "(echo \"$PARAM_KEYRING_OS_PASS\"; echo \"$PARAM_KEYRING_OS_PASS\") | cudos-noded keys add empty --keyring-backend os" 2>&1);
    if [ "$?" != 0 ]; then
    ...
    dockerResult=$(docker container exec "$startContainerName" /bin/bash -c "(echo \"$PARAM_VALIDATOR_MNEMONIC\"; echo \"$PARAM_KEYRING_OS_PASS\") | cudos-noded keys add validator --recover --keyring-backend os" 2>&1);
    if [ "$?" != 0 ]; then
    ...
    dockerResult=$(docker container exec "$startContainerName" /bin/bash -c "(echo \"$PARAM_KEYRING_OS_PASS\") | cudos-noded keys delete empty --keyring-backend os -y" 2>&1);
    ...
    dockerResult=$(docker container exec "$startContainerName" /bin/bash -c "cudos-noded collect-gentxs" 2>&1);
    ...
    do
        sleep 1
        dockerResult=$(docker exec "$startContainerName" /bin/bash -c "cudos-noded status |& tee /tmp/cudos-status" 2>&1)
        if [ "$?" != 0 ]; then
    ...
    result=$(docker container exec "$startContainerName" /bin/bash -c "cudos-noded export |& tee $tmpFilePath" 2>&1)
    EXPORTED_GENESIS=$(docker container exec "$startContainerName" /bin/bash -c "cat $tmpFilePath && rm -f $tmpFilePath")
    validJson=$(echo $EXPORTED_GENESIS | jq -e . 2>&1)
    if [ "$?" != 0 ]; then
        echo -e "${STYLE_RED}Error:${STYLE_DEFAULT} Invalid genesis $?: ${EXPORTED_GENESIS}";
        exit 1;
    fi;
    # reset the data
    result=$(docker container exec "$startContainerName" /bin/bash -c "cudos-noded unsafe-reset-all" 2>&1)
    # stop the docker
    dockerResult=$(docker stop "$startContainerName");
