...
do
    ...
    if (( freeSpaceInKiB < freeSpaceRequirementInKiB )); then
        echo -ne "${STYLE_ORANGE}Warning:${STYLE_DEFAULT} The instance $ip:$port has less than $freeSpaceRequirementInKiB KiB free space available (Available = $freeSpaceInKiB KiB)...";
        # exit 1;
    fi
done
