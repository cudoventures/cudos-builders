...
for dataGenesisPath in ./*; do
    ...
    if [ $(hasGenesisInStaking "$WORKING_DATA_GENESIS_DIR/$dataGenesisPath") = "0" ]; then
        echo -e "${STYLE_RED}Error:${STYLE_DEFAULT} Remove $dataGenesisPath because it is not included in staking.json and then re-run the tests";
        exit 1
    fi
done
