...
ETH_PRIV_KEY=$((echo "y"; echo $KEYRING_OS_PASS;) | ethermintd keys unsafe-export-eth-key zero-account --home ${CUDOS_HOME})
echo ETH_PRIV_KEY: $ETH_PRIV_KEY |& tee -a "${CUDOS_HOME}/zero-account.wallet"
...
for i in $(seq 1 $NUMBER_OF_VALIDATORS); do
    ...
    ETH_PRIV_KEY=$((echo "y"; echo $KEYRING_OS_PASS;) | ethermintd keys unsafe-export-eth-key "validator-$i" --home ${CUDOS_HOME})
    echo ETH_PRIV_KEY: $ETH_PRIV_KEY |& tee -a "${CUDOS_HOME}/validator-$i.wallet"
...
if [ "$FAUCET_BALANCE" != "" ] && [ "$FAUCET_BALANCE" != "0" ]; then
    ...
    ETH_PRIV_KEY=$((echo "y"; echo $KEYRING_OS_PASS;) | ethermintd keys unsafe-export-eth-key faucet --home ${CUDOS_HOME})
    echo ETH_PRIV_KEY: $ETH_PRIV_KEY |& tee -a "${CUDOS_HOME}/faucet.wallet"
...
# (echo $KEYRING_OS_PASS; echo $KEYRING_OS_PASS) | ethermintd gentx validator-1 "${VALIDATOR_BALANCE}${BOND_DENOM}" --chain-id $DEBUG_CHAIN_ID --keyring-backend os --home ${CUDOS_HOME}
